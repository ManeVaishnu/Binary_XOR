...
def bidirectional_dijkstra(graph, start, goal):
    ...
    total_distance = sum(graph[u][v] for u, v in zip(path, path[1:]))
...
if __name__ == "__main__":
    ...
    M = int(input("Enter the number of road segments: "))

    graph = {}
    positions = {}
    print("Enter road segment details (source, destination, weight):")
    for _ in range(M):
    ...
    path, dist = bidirectional_dijkstra(graph, start_node, end_node)
